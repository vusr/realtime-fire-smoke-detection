...
====================================
GPU-optimised quantization and evaluation pipeline for the RTDETR fire/smoke
...
--------------------------
  [1]  PyTorch  FP32   – baseline, native CUDA (batch=16)
  [2]  PyTorch  FP32   – baseline, native CUDA (batch=32)
  [3]  PyTorch  FP32   – baseline, native CUDA (batch=64)
  [4]  PyTorch  FP16   – half-precision Tensor Core inference (batch=16)
  [5]  PyTorch  FP16   – half-precision Tensor Core inference (batch=32)
  [6]  PyTorch  FP16   – half-precision Tensor Core inference (batch=64)
  [7]  TensorRT FP16   – TRT engine with layer fusion + kernel tuning (batch=16)
  [8]  TensorRT FP16   – TRT engine with layer fusion + kernel tuning (batch=32)
  [9]  TensorRT FP16   – TRT engine with layer fusion + kernel tuning (batch=64)
  [10] TensorRT INT8   – TRT engine with PTQ calibration (batch=16)
  [11] TensorRT INT8   – TRT engine with PTQ calibration (batch=32)
  [12] TensorRT INT8   – TRT engine with PTQ calibration (batch=64)
...
----------------------------------------------
  rtdetr_best_fp16_bs16.engine   TensorRT FP16 engine (batch=16)
  rtdetr_best_fp16_bs32.engine   TensorRT FP16 engine (batch=32)
  rtdetr_best_fp16_bs64.engine   TensorRT FP16 engine (batch=64)
  rtdetr_best_int8_bs16.engine   TensorRT INT8 engine (batch=16, with calibration)
  rtdetr_best_int8_bs32.engine   TensorRT INT8 engine (batch=32, with calibration)
  rtdetr_best_int8_bs64.engine   TensorRT INT8 engine (batch=64, with calibration)
...
Usage
-----
    python quantize_and_evaluate_gpu_rtdetr.py
"""
...
# ── Configuration ─────────────────────────────────────────────────────────────
MODEL_PATH    = "rtdetr_best.pt"    # trained PyTorch weights for RTDETR
...
OUTPUT_DIR    = "optimized_models_rtdetr"  # all exported model files land here
REPORT_FILE   = "quantization_report.txt"   # saved comparison report

IMG_SIZE      = 640
# Batch sizes - we'll evaluate all models with these three batch sizes
BATCH_SIZES = [16, 32, 64]  # PyTorch FP32, FP16, and TensorRT models
...
# ─── [1-6] PyTorch FP32 / FP16 (multiple batch sizes) ────────────────────────

def eval_pytorch_fp32(batch_size: int, step: str) -> dict:
    from ultralytics import RTDETR
    _banner(step, f"PyTorch FP32  (baseline, batch={batch_size})", f"cuda:{DEVICE_GPU}")
...
def eval_pytorch_fp16(batch_size: int, step: str) -> dict:
    from ultralytics import RTDETR
    _banner(step, f"PyTorch FP16  (half-precision, batch={batch_size})", f"cuda:{DEVICE_GPU}")
    model = RTDETR(MODEL_PATH)
    r = model.val(
        data=DATASET_YAML, imgsz=IMG_SIZE, batch=batch_size,
...
# ─── [7-12] TensorRT FP16 / INT8 (multiple batch sizes) ──────────────────────

def _trt_path(tag: str, batch_size: int) -> str:
    return str(Path(OUTPUT_DIR) / f"rtdetr_best_{tag}_bs{batch_size}.engine")
...
def print_report(metrics: dict[str, dict], sizes: dict[str, float], n_test: int) -> str:
    ...
    col_w   = 17
    name_w  = 20  # Increased to accommodate longer names like PT-FP32-B64
    ...
    lines.append("\n\n" + "═" * total_w)
    lines.append(f"  QUANTIZATION COMPARISON — Fire & Smoke Detection (RT-DETR) — NVIDIA L4 GPU")
    ...
    # Delta table versus PyTorch FP32 baseline (batch=16)
    base   = metrics.get("PT-FP32-B16", {})
    base_sz = sizes.get("PT-FP32-B16", float("nan"))
    if base and any(k != "PT-FP32-B16" and v for k, v in metrics.items()):
        lines.append(f"\n  Δ relative to PT-FP32-B16 baseline  (+) = improvement for accuracy / throughput")
        lines.append(sep)
        for name, m in metrics.items():
            if name == "PT-FP32-B16" or not m:
                continue
            sz   = sizes.get(name, float("nan"))
...
def main() -> None:
    banner = "RT-DETR GPU Quantization Evaluation — Fire/Smoke (NVIDIA L4, CC 8.9)"
    print("╔" + "═" * (len(banner) + 4) + "╗")
    print(f"║  {banner}  ║")
    print("╚" + "═" * (len(banner) + 4) + "╝")
    ...
    try:
        ...
        # ── PyTorch FP32 (multiple batch sizes) ──────────────────────────────────
        step_idx = 1
        for batch_size in BATCH_SIZES:
            key = f"PT-FP32-B{batch_size}"
            metrics[key] = _safe_run(
                lambda bs=batch_size, s=step_idx: eval_pytorch_fp32(bs, f"[{s}/12]"),
                f"PyTorch FP32 (batch={batch_size})"
            )
            sizes[key] = _file_mb(MODEL_PATH)
            step_idx += 1

        # ── PyTorch FP16 (multiple batch sizes) ──────────────────────────────────
        for batch_size in BATCH_SIZES:
            key = f"PT-FP16-B{batch_size}"
            metrics[key] = _safe_run(
                lambda bs=batch_size, s=step_idx: eval_pytorch_fp16(bs, f"[{s}/12]"),
                f"PyTorch FP16 (batch={batch_size})"
            )
            sizes[key] = _file_mb(MODEL_PATH)   # same .pt file, different compute
            step_idx += 1
        ...
        for batch_size in BATCH_SIZES:
            ...
            metrics[key] = _safe_run(
                lambda bs=batch_size, s=step_idx: eval_trt_fp16(bs, f"[{s}/12]"),
        ...
        for batch_size in BATCH_SIZES:
            ...
            metrics[key] = _safe_run(
                lambda bs=batch_size, s=step_idx: eval_trt_int8(bs, f"[{s}/12]"),
...
#    pip install ultralytics          # RT-DETR model + export + val pipeline
...
#    print('RTDETR available:', RTDETR is not None)
#    "
# =============================================================================